...
func (app *application) userLoginPost(w http.ResponseWriter, r *http.Request) {
	...
	// 현재 세션에서 RenewToken() 메서드를 사용하여
	// 세션 ID. 사용자에 대한 인증 상태 또는 권한 수준이 변경되면(예: 로그인 및 로그아웃 작업)
...
func (app *application) accountPasswordUpdatePost(w http.ResponseWriter, r *http.Request) {
	...
	form.CheckField(validator.MinChars(form.NewPassword, 8), "newPassword", "This field must be a least 8 characters long")
	...
	if err != nil {
		if errors.Is(err, models.ErrInvalidCredentials) {
			...
			app.render(w, http.StatusUnprocessableEntity, "password.tmple", data)
		} else if err != nil {
			app.serverError(w, err)
		}
		return
